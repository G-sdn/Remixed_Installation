...
byte ip[] = { 192, 168, 0, 103 };                     // the IP adress of your device, that should be in same universe of the network you are using
...
void setLEDS(int startled) {
  ...
  for (int i = start_address; i < number_of_channels; i = i + 3) {
    if (j <= NUM_LEDS) {
      leds[j].g = byte(packetBuffer[i + art_net_header_size + 1]);
      leds[j].r = byte(packetBuffer[i + art_net_header_size + 2]);
